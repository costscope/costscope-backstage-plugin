...
export type ProviderInfo = (
  __ContractsPaths['/providers'] extends { get: { responses: { 200: { content: { 'application/json': infer R } } } } }
    ? R
    : unknown
) extends Array<infer P> ? P : { id: string; name: string; status?: string };
...
export type DatasetSearchRow = (
  __ContractsPaths['/datasets/search'] extends { get: { responses: { 200: { content: { 'application/json': infer R } } } } }
    ? R
    : unknown
) extends Array<infer S> ? S : DatasetMeta;

/** Health response. */
export type Healthz = __ContractsPaths['/healthz'] extends { get: { responses: { 200: { content: { 'application/json': infer R } } } } }
  ? R
  : { status: string };
...
export interface CostscopeApi {
  ...
  getProviders(options?: { refresh?: boolean; signal?: AbortSignal; validate?: boolean }): Promise<ProviderInfo[]>;
  getDatasets(options?: { refresh?: boolean; project?: string; signal?: AbortSignal; validate?: boolean }): Promise<DatasetMeta[]>;
  searchDatasets(params: DatasetSearchParams & { signal?: AbortSignal; refresh?: boolean; validate?: boolean }): Promise<DatasetSearchRow[]>;
